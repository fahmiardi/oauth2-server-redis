...
class RedisRefreshTokenTest extends PHPUnit_Framework_TestCase {
    ...
    {
        $this->redis->shouldReceive('get')->once()->with('oauth:refresh:tokens:foo')->andReturn('{"id":"foo","expire_time":1,"access_token_id":"bar"}');
    ...
    public function testDeleteRefreshTokenEntity()
    {
        ...
        $token = (new RefreshTokenEntity($this->server))->setId('foo');
